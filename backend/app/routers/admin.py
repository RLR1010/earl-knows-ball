...
async def get_prediction_stats(
    ...
    for r in rows.fetchall():
        ...
        raw_rows = await db.execute(_sa_text(f"""
            ...
            JOIN {schema}.games g ON g.id = gp.game_id
            JOIN {schema}.seasons s ON s.id = g.season_id
            WHERE gp.source = 'api' AND s.year = {r.year}
...
def _get_nba_model_detail() -> SportModelDetailOut:
    """Build NBA model detail from backtest results file."""

    _NBA_FEATURE_DESCRIPTIONS = {
        "__desc__": "NBA ATS model. Predicts point differential using opponent-adjusted scoring (10/20), betting market odds, rest, and form streaks. 2020+ line data only.",
    ...
    if not results:
        return SportModelDetailOut(
            ...
            overall_mae=0, overall_ats=ModelBettingOut(correct=0, incorrect=0, total=0, pct=0),
            overall_ou=None, overall_ml=None, monthly=[], high_confidence=[],
            feature_importance_plot=[], last_updated="2026-06-12",
        )

    cat_lookup = {}
    for cat, feats in _NBA_FEATURE_CATEGORIES.items():
        for f in feats:
            cat_lookup[f] = cat
    ...
    for feat_name, imp in sorted_feats:
        features.append(ModelFeatureOut(
            name=feat_name,
            description=_NBA_FEATURE_DESCRIPTIONS.get(feat_name, feat_name),
            importance=round(imp, 4),
            category=cat_lookup.get(feat_name, "Other"),
        ))

    defined_feats = set()
    for feats_list in _NBA_FEATURE_CATEGORIES.values():
        defined_feats.update(feats_list)
    existing = set(f.name for f in features)
    for fn in sorted(defined_feats):
        if fn not in existing:
            features.append(ModelFeatureOut(
                name=fn, description=_NBA_FEATURE_DESCRIPTIONS.get(fn, fn),
                importance=0, category=cat_lookup.get(fn, "Other"),
            ))

    fi_plot = [{"name": f.name, "importance": f.importance} for f in features[:15]]

    feature_cats = []
    for cat_name, feat_names in _NBA_FEATURE_CATEGORIES.items():
        cat_import = sum(f.importance for f in features if f.name in feat_names)
        feature_cats.append({
            "name": cat_name, "feature_count": len(feat_names),
            "total_importance": round(cat_import, 4), "features": feat_names,
        })

    all_mae = [r.get("mae", 0) for r in results]
    overall_mae = round(sum(all_mae) / max(len(all_mae), 1), 2)

    def _sum_metric(key, sub):
        return sum(r.get(key, {}).get(sub, 0) for r in results if r.get(key))
    ...
    overall_ats = ModelBettingOut(
        ...
        pct=round(100 * ats_correct / max(ats_total, 1), 1), pushes=ats_pushes,
    )

    training_years = set()
    test_years = set()
    for r in results:
        test_years.add(r.get("test_year"))
        for y in r.get("train_years", []):
            training_years.add(y)

    return SportModelDetailOut(
        sport="nba",
        model_type="XGBoost Point Differential Regressor",
        description=(
            "The NBA prediction model uses XGBoost to predict point differential "
            "(home_score - away_score). Features focus on rolling scoring rates, "
            "rest/scheduling (especially back-to-backs), home/away splits, market "
            "probabilities, and recent ATS form.\n\n"
            "Trained exclusively on games with complete line data from 2020+. "
            "The initial feature set emphasizes proven NBA handicapping factors:"
            " scoring momentum, rest advantage, home court, market implied odds,"
            " spread movement, and cover streaks."
        ),
        algorithm="XGBoost Regressor (n_estimators=350, max_depth=5, learning_rate=0.06)",
        ...
        monthly=[],
        high_confidence=[
            {"threshold": pct, "total": int(ats_total * pct / 100),
             "correct": int(ats_correct * pct / 100),
             "pct": round(100 * ats_correct / max(ats_total, 1), 1)}
