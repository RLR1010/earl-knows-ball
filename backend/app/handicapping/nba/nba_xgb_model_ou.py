...
# ── Training defaults ───────────────────────────────────────────────────────────
# Aligned with the MLB OU trainer (mlb_xgb_model_ou.py) so NBA shares the same
# boosting settings.
DEFAULT_N_ESTIMATORS = 500
DEFAULT_LEARNING_RATE = 0.04
DEFAULT_MAX_DEPTH = 5
DEFAULT_SUBSAMPLE = 0.8
DEFAULT_COL_SAMPLE = 0.7
DEFAULT_REG_LAMBDA = 1.5
DEFAULT_GAMMA = 0.1
...
def _compute_decay_weights(
    ...
    """Assign higher weight to more recent seasons (same as mlb_xgb_model_ou)."""
...
async def train_model(
    ...
    for test_year in TEST_YEARS:
        ...
        # Early stopping (matches MLB + NBA ATS): hold out the MOST RECENT ~15%
        # of the training period as a time-ordered eval set (no leakage — the
        # model predicts a later period). xgb.train (native API) still accepts
        # early_stopping_rounds + evals in XGB 3.x.
        ...
        if not df_test.empty and len(df_test) > 0:
            ...
            # training years). Missing cols are filled with NaN then imputed.
