...
export default function NBAGameTabs({ gameId, prediction }: NBAGameTabsProps) {
  ...
  function DetailedStatsTab({ gameId }: { gameId: number }) {
    ...
    // using the pick_card_section attached read-time from the nba.features table.
    ...
    function renderSectionedFeatures(section: string) {
      ...
      // Order by the admin-chosen sort_order column (from nba.features, attached
    ...
    return (
      <div className="space-y-6 text-xs">
        ...
        {/* Sectioned stats — grouped by pick_card_section from the NBA
